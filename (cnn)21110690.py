...
x_train1_cnn = x_train1.reshape((x_train1.shape[0], x_train1.shape[1], 1))
x_val1_cnn = x_val1.reshape((x_val1.shape[0], x_val1.shape[1], 1))

model_cnn1 = Sequential()
model_cnn1.add(Conv1D(filters=32, kernel_size=4, activation='relu',
                      input_shape=(x_train1_cnn.shape[1], x_train1_cnn.shape[2])))
...
model_cnn1.add(Dense(120, activation="relu"))
model_cnn1.add(Dense(120, activation="relu"))
model_cnn1.add(Dense(120, activation="relu"))
model_cnn1.add(Dense(80, activation="relu"))
model_cnn1.add(Dense(50, activation="relu"))
...
model_cnn1.compile(loss=keras.losses.SparseCategoricalCrossentropy(from_logits=True),
...
cnn_history1 = model_cnn1.fit(x_train1_cnn, y_train1, batch_size=32, epochs=1000, validation_data=(x_val1_cnn, y_val1),
                              callbacks=[tensorboard_callback, early_stopping_callback], verbose=2)

cnn_score1 = model_cnn1.evaluate(x_test1, y_test1, verbose=0)

print("Test1 loss:", cnn_score1[0])
print("Test1 accuracy:", round(cnn_score1[1] * 100, 3))
